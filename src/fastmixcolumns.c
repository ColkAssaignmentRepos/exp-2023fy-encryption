//
// Created by Manaki ITO on 2023/06/23.
//

#include "aes128.h"

// Factor: 0x02
unsigned mbox02[256] = {
        
};

// Factor: 0x03
unsigned mbox03[256] = {
        
};

void MixColumns(unsigned char state[4 * Nb]) {
    unsigned char res[4 * Nb];

    for (int i = 0; i < (12 + 1); i += 4) {
        res[i] = (unsigned char) (mbox02[state[i]] ^ mbox03[state[i + 1]] ^ state[i + 2] ^ state[i + 3]);
        res[i + 1] = (unsigned char) (state[i] ^ mbox02[state[i + 1]] ^ mbox03[state[i + 2]] ^ state[i + 3]);
        res[i + 2] = (unsigned char) (state[i] ^ state[i + 1] ^ mbox02[state[i + 2]] ^ mbox03[state[i + 3]]);
        res[i + 3] = (unsigned char) (mbox03[state[i]] ^ state[i + 1] ^ state[i + 2] ^ mbox02[state[i + 3]]);
    }

    for (int i = 0; i < 4 * Nb; i++) {
        state[i] = res[i];
    }
}
